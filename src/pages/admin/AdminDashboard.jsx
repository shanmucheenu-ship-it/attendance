              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead className="bg-slate-50 text-slate-500 uppercase text-xs">
                  <tr className="border-b border-slate-100">
                    <th rowSpan="2" className="px-4 py-3.5 text-left align-middle border-r border-slate-100 font-bold">Department</th>
                    <th colSpan="3" className="px-4 py-2 text-center border-r border-slate-100 font-bold">2nd Year</th>
                    <th colSpan="3" className="px-4 py-2 text-center border-r border-slate-100 font-bold">3rd Year</th>
                    <th rowSpan="2" className="px-4 py-3.5 text-center align-middle border-r border-slate-100 font-bold bg-slate-100/50">Total Absentees</th>
                    <th rowSpan="2" className="px-4 py-3.5 text-center align-middle font-bold">History</th>
                  </tr>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    <th className="px-2 py-2 text-center border-r border-slate-100 font-semibold text-slate-500">Single</th>
                    <th className="px-2 py-2 text-center border-r border-slate-100 font-semibold text-indigo-600">Sec A</th>
                    <th className="px-2 py-2 text-center border-r border-slate-100 font-semibold text-indigo-600">Sec B</th>
                    <th className="px-2 py-2 text-center border-r border-slate-100 font-semibold text-slate-500">Single</th>
                    <th className="px-2 py-2 text-center border-r border-slate-100 font-semibold text-indigo-600">Sec A</th>
                    <th className="px-2 py-2 text-center border-r border-slate-100 font-semibold text-indigo-600">Sec B</th>
                  </tr>
                ...
                <tbody className="divide-y divide-slate-100">
                  {depts.map((d, idx) => {
                    ...
                    const deptTotal = getDeptTotal(d);
                    const hasSubmittedToday = forwardedSubmissions.some(s => s.department === d);
                    
                    return (
                      <tr key={d} className={isEven ? 'bg-white hover:bg-slate-50/20' : 'bg-slate-50/30 hover:bg-slate-50/50'}>
                        <td className="px-4 py-3 font-bold text-slate-800 border-r border-slate-100">{d}</td>
                        <td className="px-2 py-3 text-center font-bold text-slate-500 border-r border-slate-100 bg-slate-50/30">{d === 'Computer' ? '-' : getSubAbsent(d, '2nd Year', 'Single')}</td>
                        <td className="px-2 py-3 text-center font-bold text-red-500 border-r border-slate-100 bg-red-50/5">{d === 'Computer' ? getSubAbsent(d, '2nd Year', 'A') : '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-red-500 border-r border-slate-100 bg-red-50/5">{d === 'Computer' ? getSubAbsent(d, '2nd Year', 'B') : '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-slate-500 border-r border-slate-100 bg-slate-50/30">{d === 'Computer' ? '-' : getSubAbsent(d, '3rd Year', 'Single')}</td>
                        <td className="px-2 py-3 text-center font-bold text-red-500 border-r border-slate-100 bg-red-50/5">{d === 'Computer' ? getSubAbsent(d, '3rd Year', 'A') : '-'}</td>
                        <td className="px-2 py-3 text-center font-bold text-red-500 border-r border-slate-100 bg-red-50/5">{d === 'Computer' ? getSubAbsent(d, '3rd Year', 'B') : '-'}</td>
                        <td className="px-2 py-3 text-center font-extrabold text-slate-900 border-r border-slate-100 bg-slate-100/30">{deptTotal}</td>
                        <td className="px-4 py-3 text-center flex items-center justify-center gap-2">
                          <button 
                            onClick={() => { setHistoryView({ department: d, year: '3rd Year', section: 'A' }); }}
                          ...
                          {!hasSubmittedToday && (
                            <button
                              onClick={() => {
                                sendAlert(d, 'Attendance has not been submitted for today.');
                                showToast(`Alert sent to ${d} HOD`, 'success');
                              }}
                  ...
                  <tr className="bg-slate-100/80 font-bold border-t-2 border-slate-200">
                    ...
                    <td className="px-2 py-3 text-center text-slate-600 border-r border-slate-100 font-extrabold bg-slate-50/30">{getColTotal('2nd Year', 'Single')}</td>
                    <td className="px-2 py-3 text-center text-red-600 border-r border-slate-100 font-extrabold">{getColTotal('2nd Year', 'A')}</td>
                    <td className="px-2 py-3 text-center text-red-600 border-r border-slate-100 font-extrabold">{getColTotal('2nd Year', 'B')}</td>
                    <td className="px-2 py-3 text-center text-slate-600 border-r border-slate-100 font-extrabold bg-slate-50/30">{getColTotal('3rd Year', 'Single')}</td>
                    <td className="px-2 py-3 text-center text-red-600 border-r border-slate-100 font-extrabold">{getColTotal('3rd Year', 'A')}</td>
                    <td className="px-2 py-3 text-center text-red-600 border-r border-slate-100 font-extrabold">{getColTotal('3rd Year', 'B')}</td>
                    <td className="px-2 py-3 text-center text-slate-900 border-r border-slate-100 font-black bg-slate-200/60">{getGrandTotal()}</td>
