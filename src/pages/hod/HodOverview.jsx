...
const HodOverview = () => {
  const { auth, students, attendance, clearSubmissions, showToast, approveSubmission, rejectSubmission, sendAlert, updateSubmissionCount } = useContext(AppContext);
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {deptSubmissions
                    ...
                    .map(sub => {
                      const isEditing = editingHistoryCounts[sub.id] !== undefined;
                      const currentVal = isEditing ? editingHistoryCounts[sub.id] : sub.absenteesCount;
                      return (
                        <tr key={sub.id} className="hover:bg-slate-50/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-center">
                            {isEditing ? (
                              ...
                              <span className="font-semibold text-slate-700">{sub.absenteesCount}</span>
                            )}
                          </td>
                          <td className="px-4 py-3 text-center">
                            ...
                            {sub.status === 'Approved' && (
                              isEditing ? (
                                <div className="space-x-2 inline-flex items-center justify-end">
                                  <button
                                    onClick={async () => {
                                      await updateSubmissionCount(sub.id, currentVal);
                                      setEditingHistoryCounts(prev => {
                                        const updated = { ...prev };
                                        delete updated[sub.id];
                                        return updated;
                                      });
                                    }}
                                    className="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-2.5 py-1.5 rounded-lg transition-colors shadow-sm cursor-pointer"
                                  >
                                    Save
                                  </button>
                                  <button
                                    ...
                                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer border border-slate-200"
                                  >
                                    Cancel
                                  </button>
                                </div>
                                ...
                                <button
                                  onClick={() => {
                                    setEditingHistoryCounts(prev => ({
                                      ...prev,
                                      [sub.id]: sub.absenteesCount
                                    }));
                                  }}
                                  className="text-indigo-600 hover:text-indigo-800 text-xs font-bold border border-indigo-200 hover:border-indigo-400 bg-indigo-50/20 hover:bg-indigo-50/50 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                                >
                                  Update Count
                                </button>
                              )
